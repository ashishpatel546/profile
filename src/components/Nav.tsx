...
export default function Nav() {
  ...
  return (
    <header className={`no-print fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${chrome}`}>
      ...
      <nav className="gutter flex h-[72px] items-center justify-between gap-4" aria-label="Primary">
        ...
        >
          <span className="whitespace-nowrap font-display text-xl leading-none text-ink transition-colors group-hover:text-accent">
            {person.name}
          </span>
        ...
        <div className="flex shrink-0 items-center gap-2">
            ...
            {SECTIONS.map((s) => (
              <li key={s.id}>
                <a
                  ...
                  className={`relative block cursor-pointer whitespace-nowrap px-3 py-2.5 text-[0.95rem] font-medium transition-colors duration-200 ${
                    active === s.id ? 'text-accent' : 'text-muted hover:text-ink'
                  }`}
                >
                  {s.label}
                  {active === s.id && (
                    <span aria-hidden="true" className="absolute inset-x-3 bottom-1 h-px bg-accent" />
                  )}
                </a>
              </li>
            ))}
          ...
          <a
            ...
            className="hidden cursor-pointer whitespace-nowrap border border-line-strong px-4 py-2.5 text-[0.95rem] font-medium text-ink transition-colors duration-200 hover:border-accent hover:text-accent md:inline-block"
          >
            Résumé
          </a>
          ...
          <button
            ...
            className="inline-flex h-11 w-11 cursor-pointer items-center justify-center border border-line text-ink transition-colors hover:border-accent hover:text-accent lg:hidden"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
      ...
      <div id="mobile-nav" hidden={!menuOpen} className="border-t border-line bg-canvas lg:hidden">
